"""
mans-bots v5 (laika limits + sesiju filtrs), bāze v4: 20 tirgi x 4 stratēģijas x 4 laika periodi (M5, M15, M30, H1),
ātruma filtrs un atbalsta/pretestības līmeņi.
...
SESSIONS = {
    "US500": ("America/New_York", (9, 30), (16, 0)),
    "NAS100": ("America/New_York", (9, 30), (16, 0)),
    "US30": ("America/New_York", (9, 30), (16, 0)),
    "GER40": ("Europe/Berlin", (9, 0), (17, 30)),
}
MAX_COST_PCT = 0.30
...
M5 = 5
# laika periods: (cTrader kods, cik stundas vēstures)
TFS = {"m5": (5, 26), "m15": (7, 72), "m30": (8, 144), "h1": (9, 288)}
BUY, SELL = 1, 2
MARKET = 1
...
def combine(signals):
    ...
    for side, lst in by_side.items():
        if lst:
            ...
            fam = sorted({n.split("@")[0] for n, _ in lst})
            totals[side] = (t, "+".join(fam), ", ".join(sorted(n for n, _ in lst)))
    if not totals:
        return None
    side = max(totals, key=lambda s: totals[s][0])
    t, tag, detail = totals[side]
    other = totals.get(SELL if side == BUY else BUY)
    if other:
        t -= 0.5 * other[0]
    if t <= 0:
        return None
    return side, t, tag, detail


def in_session(label):
    if label not in SESSIONS:
        return True
    tzname, (h1, m1), (h2, m2) = SESSIONS[label]
    now = datetime.now(timezone.utc).astimezone(ZoneInfo(tzname))
    if now.weekday() >= 5:
        return False
    mins = now.hour * 60 + now.minute
    return h1 * 60 + m1 <= mins < h2 * 60 + m2 - 30   # pēdējās 30 min neatver


async def fetch_bars(c, aid, sid, period, hours):
    now_ms = int(time.time() * 1000)
    tb = await c.request(TRENDBARS_REQ, {
        "ctidTraderAccountId": aid, "symbolId": sid, "period": period,
        "fromTimestamp": now_ms - hours * 3600 * 1000, "toTimestamp": now_ms,
    }, TRENDBARS_RES)
    raw = sorted(tb.get("trendbar", []), key=lambda b: b["utcTimestampInMinutes"])[:-1]
    return [(b["utcTimestampInMinutes"],
             (b["low"] + b.get("deltaHigh", 0)) / 100000,
             b["low"] / 100000,
             (b["low"] + b.get("deltaClose", 0)) / 100000) for b in raw]


def key_levels(bars, k=3, last=150):
    """Atbalsta/pretestības līmeņi no H1 svārstību punktiem."""
    b = bars[-last:]
    sh, sl = swings(b, k)
    return [b[i][1] for i in sh], [b[i][2] for i in sl]


def htf_trend(bars):
    if len(bars) < 60:
        return None
    closes = [x[3] for x in bars]
    return BUY if ema(closes, 21) > ema(closes, 50) else SELL
...
async def run():
    ...
    async with websockets.connect(HOST, ping_interval=20, max_size=None) as ws:
        ...
        if MODE != "test" and bot_pos:
            ...
            still = []
            for p in bot_pos:
                opened = int(p.get("tradeData", {}).get("openTimestamp", now_ms))
                age_min = (now_ms - opened) / 60000
                if age_min > MAX_HOLD_MIN:
                    await c.request(CLOSE_POS_REQ, {
                        "ctidTraderAccountId": aid, "positionId": int(p["positionId"]),
                        "volume": int(p["tradeData"]["volume"])}, EXECUTION_EVENT)
                    notify(f"⏱️ Laika limits: aizvēru pozīciju pēc {age_min / 60:.1f} h "
        ...
        # --- Skenē: visi tirgi x visas stratēģijas x visi laika periodi ---
        px, scanned, skipped = {}, [], []
        for label, sid, quote in found:
            tf = {}
            for key, (period, hours) in TFS.items():
                await asyncio.sleep(0.3)
                tf[key] = await fetch_bars(c, aid, sid, period, hours)
            m5 = tf["m5"]
            if len(m5) < 60:
                skipped.append(f"{label}: par maz datu ({len(m5)})")
                continue
            px[label] = m5[-1][3]
            if time.time() / 60 - m5[-1][0] > 20:
                skipped.append(f"{label}: tirgus slēgts")
                continue
            if not in_session(label):
                skipped.append(f"{label}: ārpus galvenās sesijas")
                continue
            a5 = atr(m5)
            if a5 <= 0:
            ...
            for key in ("m5", "m15"):
                ...
                for name, fn in STRATEGIES:
                    if name == "breakout" and key != "m5":
                        continue
                    try:
                        r = fn(bars, a)
                    except Exception as e:
                        print(f"{label}/{name}@{key} kļūda: {e}")
                        r = None
                    if r:
                        signals.append((f"{name}@{key}", r[0], r[1]))
            combo = combine(signals)
            if not combo:
                continue
            side, score, tag, detail = combo

            # Lielāko periodu (M30, H1) trends: sakrīt = bonuss, pretī = sods
            htf_ok = 0
            for key in ("m30", "h1"):
                t = htf_trend(tf[key])
                if t is None:
                ...
                if t == side:
                    ...
                    score *= 0.6
            res, sup = key_levels(tf["h1"]) if len(tf["h1"]) >= 20 else ([], [])
            scanned.append({"label": label, "sid": sid, "quote": quote, "side": side,
                            "score": score, "tag": tag, "detail": detail, "htf": htf_ok,
                            "price": m5[-1][3], "atr5": a5, "res": res, "sup": sup})

        # --- Marža, izmērs, TP/SL, ātrums, līmeņi ---
        candidates = []
        for s in sorted(scanned, key=lambda x: x["score"], reverse=True)[:10]:
            ...
            tp_dist = tp_rel / 100000

            # Ātruma filtrs: cik "tipisku 5 min kustību" līdz TP
            speed = tp_dist / s["atr5"]
            if speed > MAX_TP_ATR:
                skipped.append(f"{s['label']}: par lēnu (TP = {speed:.1f}x ATR)")
                continue
            s["score"] *= min(1.5, 2.0 / speed)

            # Līmeņi: pretestība/atbalsts ceļā uz TP
            p = s["price"]
            blocked = (any(p < r < p + tp_dist for r in s["res"]) if s["side"] == BUY
                       else any(p - tp_dist < x < p for x in s["sup"]))
            if blocked:
                s["score"] *= 0.5
            s.update(volume=volume, units=units, eq=eq, tp_rel=tp_rel, sl_rel=sl_rel,
                     margin=margin_min * volume / min_vol, speed=speed, blocked=blocked)
            candidates.append(s)
        ...
        if MODE == "test":
            lines = [f"🧪 TESTS OK (v5) | virtuālais {virtual:.2f}€ (reālais {balance:.2f}€)",
            ...
            for s in sorted(scanned, key=lambda x: x["score"], reverse=True)[:8]:
                extra = ""
                if "speed" in s:
                    extra = f", ātrums {s['speed']:.1f}" + (", līmenis ceļā" if s["blocked"] else "")
                lines.append(f"  {s['label']}: {ARROW[s['side']]} {s['score']:.2f} "
                             f"[{s['detail']}] HTF {s['htf']}/2{extra}")
            if not scanned:
        ...
        notify(f"🤖 Atvērts {ARROW[chosen['side']]} {chosen['label']}\n"
               f"Stratēģija: {chosen['detail']} (vērtējums {chosen['score']:.2f})\n"
               f"Lielie periodi sakrīt: {chosen['htf']}/2 | ātrums {chosen['speed']:.1f}x ATR\n"
